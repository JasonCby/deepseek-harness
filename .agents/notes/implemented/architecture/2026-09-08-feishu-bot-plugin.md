...
- Known limits recorded in the package README: best-effort reply delivery (no outbox), events lost during a transport swap or WSS disconnect, single instance per Feishu app (cluster-mode random unicast), text messages only, resumed chats use the deployment's default model route, and unencrypted webhooks rely on route secrecy. A chat whose persisted log predates session format v3 is refused on resume (the runtime's header-version check throws) and answered with the failure notice; archiving the old log lets the same chat id start a fresh v3 session.
