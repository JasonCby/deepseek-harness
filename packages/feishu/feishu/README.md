...
| `cardTemplates` | Registry of card templates for bound tool turns: name, `bindTool` (optional `workflowName` filter), exactly one of platform `templateId` or local `card` JSON carrying `{{variable}}` placeholders, and per-variable extraction rules (a `context` key or a `tool-result` dot path, `required`, `maxLength`). |
...
- `matchCardTemplate` / `resolveTemplateVariables` / `renderTemplateReply` / `convertCardV2toV1` — the pure card-template pipeline: registry matching over the turn's tool calls, variable extraction from logged tool-result meta and message facts, platform-or-local payload rendering, and the verified card JSON 2.0 → 1.0 projection.
